# IGCSE Past Papers Smart Downloader (PowerShell)
# Only downloads valid PDFs, skips error pages
...
# Paper numbers (all possible variants)
...
# Minimum valid PDF size (300 KB to avoid error pages)
$MinValidSize = 300KB
...
function Download-File {
    ...
    if (Test-Path $OutputPath) {
        ...
        if ($ExistingSize -gt $MinValidSize) {
            $script:TotalSkipped++
            return $true
        } else {
    ...
    try {
        ...
        # Check if it's a valid PDF with good size
        $FileSize = (Get-Item $OutputPath).Length
        $FileBytes = [System.IO.File]::ReadAllBytes($OutputPath)
        
        $IsPDF = $FileBytes.Length -gt 4 -and $FileBytes[0] -eq 0x25 -and $FileBytes[1] -eq 0x50 -and $FileBytes[2] -eq 0x44 -and $FileBytes[3] -eq 0x46
        $IsValidSize = $FileSize -gt $MinValidSize
        
        if ($IsPDF -and $IsValidSize) {
            $SizeKB = [math]::Round($FileSize/1KB, 2)
            Print-Status "Green" "  + Downloaded: $FileName ($SizeKB KB)"
            Log-Message "SUCCESS" "Downloaded: $OutputPath ($SizeKB KB)"
            $script:TotalDownloaded++
            return $true
        } else {
            # Not a valid PDF or too small (error page)
            Remove-Item $OutputPath -Force
            $script:TotalFailed++
            return $false
        }
...
# Main execution
Print-Status "Cyan" "=========================================================="
Print-Status "Cyan" "   IGCSE Past Papers SMART Downloader"
Print-Status "Cyan" "   Only downloads valid PDFs (>300 KB)"
Print-Status "Cyan" "   Source: PapaCambridge | Years: 2015-2025"
Print-Status "Cyan" "=========================================================="

Log-Message "INFO" "Starting smart download process"
Log-Message "INFO" "Base directory: $BaseDir"
Log-Message "INFO" "Minimum valid size: $([math]::Round($MinValidSize/1KB, 2)) KB"

$StartTime = Get-Date

# Download papers for each subject
foreach ($SubjectCode in $Subjects.Keys | Sort-Object) {
...
Print-Status "Red" "X Failed/Not found: $TotalFailed attempts"
...
Print-Status "Green" "`n+ All done! Only VALID papers saved to '$BaseDir' directory."
