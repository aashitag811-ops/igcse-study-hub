#!/usr/bin/env python3
"""
Batch Physics MCQ Parser
Automatically parses all Physics (0625) MCQ papers in the scripts directory
...
def find_physics_papers(scripts_dir="scripts"):
    """Find all Physics question papers and their corresponding marking schemes"""
    papers = []
    
    # Find all 0625 question papers (qp)
    qp_files = glob.glob(f"{scripts_dir}/0625_*_qp_*.pdf")
    
    for qp_path in sorted(qp_files):
        filename = os.path.basename(qp_path)
        # Extract paper info: 0625_m20_qp_22.pdf
...
def main():
    print("=" * 70)
    print("BATCH PHYSICS MCQ PARSER")
    print("=" * 70)
    print()
    
    # Find all Physics papers
    papers = find_physics_papers()
    print(f"Found {len(papers)} Physics MCQ papers to parse\n")
