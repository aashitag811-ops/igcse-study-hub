...
export default function ProfilePage() {
  ...
  const inkDropsRef = useRef<Array<{x:number;y:number;r:number;opacity:number;vx:number;vy:number;life:number;maxLife:number}>>([]);
  const inkCursorRef = useRef<{x:number;y:number}>({ x: -999, y: -999 });
  ...
  useEffect(() => {
    ...
    // seed initial ink drops
    const seed = () => {
      const drops = inkDropsRef.current;
      for (let i = 0; i < 18; i++) {
        const life = Math.random() * 180 + 60;
        drops.push({
    ...
    let frame = 0;
    const draw = () => {
      inkAnimRef.current = requestAnimationFrame(draw);
      frame++;
      ...
      // gold vein streaks (static, painted once via noise-like lines)
      ctx.save();
      for (let i = 0; i < 5; i++) {
        const x0 = (i * 0.21 + 0.05) * canvas.width;
        const cp1x = x0 + Math.sin(frame * 0.002 + i) * 18;
        const cp1y = canvas.height * 0.3 + i * 14;
        const cp2x = x0 + Math.cos(frame * 0.0015 + i) * 22;
        const cp2y = canvas.height * 0.7 - i * 10;
        const grd = ctx.createLinearGradient(x0, 0, x0, canvas.height);
        grd.addColorStop(0, 'rgba(201,168,76,0)');
        grd.addColorStop(0.3 + i * 0.06, `rgba(201,168,76,${0.06 + i * 0.012})`);
        grd.addColorStop(1, 'rgba(201,168,76,0)');
        ctx.beginPath();
        ctx.moveTo(x0, 0);
        ctx.bezierCurveTo(cp1x, cp1y, cp2x, cp2y, x0 + Math.sin(frame * 0.001 + i) * 12, canvas.height);
        ctx.strokeStyle = grd;
      ...
      // ink drops
      const cursor = inkCursorRef.current;
      const drops = inkDropsRef.current;
      for (let i = drops.length - 1; i >= 0; i--) {
        ...
        const targetOpacity = progress > 0.85 ? (1 - progress) / 0.15 * d.opacity
                            : progress < 0.15 ? progress / 0.15 * d.opacity
                            : d.opacity;

        if (d.life <= 0 || targetOpacity < 0.005) {
          drops.splice(i, 1);
          // respawn
        ...
        grd.addColorStop(0,   `rgba(8,6,14,${targetOpacity})`);
        grd.addColorStop(0.5, `rgba(18,12,28,${targetOpacity * 0.7})`);
        grd.addColorStop(1,   'rgba(8,6,14,0)');
        ctx.beginPath();
        ctx.ellipse(d.x, d.y, d.r * (1 + Math.sin(frame * 0.02 + i) * 0.12), d.r * (1 + Math.cos(frame * 0.018 + i) * 0.1), frame * 0.003 + i, 0, Math.PI * 2);
        ctx.fillStyle = grd;
        ctx.fill();
      }

      // spawn on cursor movement every few frames
      if (frame % 6 === 0 && cursor.x > 0) spawnDrop(cursor.x + (Math.random()-0.5)*30, cursor.y + (Math.random()-0.5)*30);
    };
    draw();
  ...
  return (
    <div style={{ minHeight: '100vh', background: BG, position: 'relative', overflowX: 'hidden' }}>
      ...
      <div style={{ position: 'relative', zIndex: 2, maxWidth: 1100, margin: '0 auto', padding: '32px 40px 60px', paddingTop: '96px' }}>
        ...
        {/* Profile card */}
        <div style={{ position: 'relative', border: `1px solid ${BORDER2}`, borderRadius: '1rem', marginBottom: '1.5rem', overflow: 'hidden', minHeight: 220 }}>
          ...
          <canvas
            ...
            onMouseMove={e => {
              ...
              inkCursorRef.current = { x: e.clientX - rect.left, y: e.clientY - rect.top };
            }}
            onMouseLeave={() => { inkCursorRef.current = { x: -999, y: -999 }; }}
            style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', display: 'block' }}
          />

          {/* Gold vignette overlay */}
          <div style={{ position: 'absolute', inset: 0, background: 'radial-gradient(ellipse at 70% 50%, rgba(201,168,76,0.07) 0%, transparent 70%)', pointerEvents: 'none' }} />
          <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(to right, rgba(10,10,14,0.55) 0%, transparent 60%)', pointerEvents: 'none' }} />

          {/* Content row */}
          <div style={{ position: 'relative', zIndex: 2, display: 'flex', justifyContent: 'space-between', alignItems: 'stretch', padding: '2rem', gap: '2rem', flexWrap: 'wrap' }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'start', marginBottom: '1.5rem', flexWrap: 'wrap', gap: '0.75rem' }}>
                ...
                <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'center' }}>
                  {CREATORS.includes(userEmail ?? '') && (
                    <button onClick={() => router.push('/igcse/admin/moderate')}
            ...
            {/* Right — Hello + name */}
            <div style={{ display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'flex-end', minWidth: 220, textAlign: 'right', paddingLeft: '1rem', borderLeft: `1px solid rgba(201,168,76,0.12)` }}>
              <span style={{
                fontFamily: "'DM Sans', system-ui, sans-serif",
                fontSize: '1rem', fontWeight: 400, letterSpacing: '0.18em',
              ...
              <span style={{
                ...
                lineHeight: 1.1,
                background: 'linear-gradient(135deg, #C9A84C 0%, #F0D080 40%, #C9A84C 70%, #9A7A30 100%)',
