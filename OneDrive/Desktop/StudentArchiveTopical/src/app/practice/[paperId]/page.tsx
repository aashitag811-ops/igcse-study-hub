        ...
        <div className="bg-zinc-900 rounded-2xl border border-white/10 shadow-lg overflow-hidden">
          ...
          {!q.imageUrl && q.options && (
            <div className="px-8 pb-6 space-y-3">
              {q.options.map(opt => (
                <div key={opt.letter} className="flex gap-3 items-baseline text-base text-slate-300">
                  <span className="font-bold w-4 flex-shrink-0 text-violet-400">{opt.letter}</span>
                  <span>{opt.text}</span>
                </div>
              ))}
            </div>
